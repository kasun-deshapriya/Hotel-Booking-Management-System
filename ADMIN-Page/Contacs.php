...
<?php
    ...
    $query = "SELECT `M_ID`, `Name`, `Email`, `Subject`, `Massage` FROM `contactus`";
    $result = mysqli_query($conn, $query);
        ...
        while ($row = mysqli_fetch_assoc($result)) {
            ...
            <button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#exampleModal">
              ...
              <a href ="DeleteContacs.php?deleteCode='.$row['M_ID'].'" class="btn1">Delete</a>
        </td>';
...
if (isset($_POST['id'])) {
    ...
    $query = "SELECT * FROM `contactus` WHERE `M_ID` = '$id'";
    $result = mysqli_query($conn, $query);
...
<form action="" method="post">
    <input type="text" class="C4" name="id" placeholder="Enter ID">
    <input type="submit" class="btn btn-primary" name="submit" value="Search">
    <input type="text" class="Con1" name="Name" value="<?php echo isset($Name) ? $Name : ''; ?>">
    <input type="email" class="Con2" name="Email" value="<?php echo isset($Email) ? $Email : ''; ?>">
    <input type="text" class="Con3" name="Subject" value="<?php echo isset($Subject) ? $Subject : ''; ?>">
    <input type="text" class="Con4" name="Message" value="<?php echo isset($Message) ? $Message : ''; ?>">
</form>
...
if (isset($_POST['submit'])) {
    ...
    $sql = "UPDATE `contactus` SET `Name`=?, `Email`=?, `Subject`=?, `Massage`=? WHERE M_ID=?";

    // Prepare the statement
    $stmt = mysqli_prepare($conn, $sql);

    // Set the value of $M_ID, assuming you have a variable for it
    $M_ID = $_POST['id']; // Replace '$_POST['id']' with the appropriate value

    // Bind parameters to the placeholders
    mysqli_stmt_bind_param($stmt, "ssssi", $Name, $Email, $Subject, $Message, $M_ID);
