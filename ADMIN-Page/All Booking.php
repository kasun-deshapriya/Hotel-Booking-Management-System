...
<?php
    ...
    $query = "SELECT `Booking_ID`, `Customer_NIC`, `Room _No`, `Date_Of_Check_in`, `Date_Of_Check_out`, `Number_Of_Guests` FROM `room booking`";
    $result = mysqli_query($conn, $query);
        ...
        while ($row = mysqli_fetch_assoc($result)) {
            ...
            echo '<td>' . $row['Room _No'] . '</td>';
            echo '<td>' . $row['Date_Of_Check_in'] . '</td>';
            echo '<td>' . $row['Date_Of_Check_out'] . '</td>';
            echo '<td>' . $row['Number_Of_Guests'] . '</td>';
            echo '<td>
            <button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#exampleModal">
            Accept
        </button>
        <a href ="DelRoomBooking.php?deleteCode='.$row['Booking_ID'].'" class="btn1">Delete</a>
          <td>';
