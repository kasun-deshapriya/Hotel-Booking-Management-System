...
<?php
    ...
    $query = "SELECT `Hall_No`, `Hall_Type`, `Hall_Capacity`, `Hall_Description`, `Hall_price` ,'Image'FROM `hall`";
    $result = mysqli_query($conn, $query);
        ...
        while ($row = mysqli_fetch_assoc($result)) {
            // Step 5: Display the data on the web page
            echo '<tr>';
            echo '<td>' . $row['Hall_No'] . '</td>';
            echo '<td>' . $row['Hall_Type'] . '</td>';
            echo '<td>' . $row['Hall_Capacity'] . '</td>';
            echo '<td>' . $row['Hall_Description'] . '</td>';
            echo '<td>' . $row['Hall_price'] . '</td>';
            echo '<td><img src="' . $row['Image'] . '" alt="Room Image"></td>';
            echo '<td>
                <button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#exampleModal">
                    Edit
                </button>
                <a href ="Delet.php?deleteCode='.$row['Hall_No'].'" class="btn1">Delete</a>
  ...
  <div class="modal-dialog" >
    <div class="modal-content">
      <div class="modal-header">
        <h1 class="modal-title fs-5" id="exampleModalLabel">Edit Hall</h1>
...
if (isset($_POST['Room_No'])) {
    ...
    if ($result && mysqli_num_rows($result) > 0) {
        ...
        $Bed = $row['Bed'];
        $Capasity = $row['Capasity'];
        $Price = $row['Price'];
        $Description = $row['Description'];
    } else {
        echo "No data found for the provided Room No.";
    }
}
?>
...
<form action="" method="post" enctype="multipart/form-data">
    <br><br><br>
    <label for="" class="label3">Hall No :</label>
    <input type="text" name="Room_No" required class="input"><br>

    <label for="" class="label3">Hall Type :</label>
    <select name="Bed" id="dropdown" class="input1">
        <option value=""></option>
        <option value="Delux">AC</option>
        <option value="Super Delux">Non-AC</option>
        
    </select><br>

    <label for="" class="label3">Capacity :</label>
    <input type="text" name="Price" required class="input2">
    <br>

    <label for="" class="label3">Hall Price :</label>
    <input type="text" name="Price" required class="input3">

    <label for="" class="label4">Hall Description :</label>
    <input type="text" name="Description" required class="input4"> <br><br>

    <label for="" class="label4">Add Image :</label>
    <input type="file" name="image" accept="image/*" class="input2"><br><br>
