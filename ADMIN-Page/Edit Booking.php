...
<?php
    ...
    $query = "SELECT `Hall_Booking_ID`, `Customer_NIC`, `Hall_ID`, `Date`, `Number_Of_Guests`, `Hall_Type`, `Hall_Name`, `Hall_Price` FROM `hall booking` ";
    $result = mysqli_query($conn, $query);
        ...
        while ($row = mysqli_fetch_assoc($result)) {
            ...
            echo '<tr>';
            echo '<td>' . $row['Hall_Booking_ID'] . '</td>';
        ...
        <a href ="DelHallBooking.php?deleteCode='.$row['Hall_Booking_ID'].'" class="btn1">Delete</a>
          <td>';
